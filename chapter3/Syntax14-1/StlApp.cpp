...
int main()
{
	...
	std::vector<RectPtr> rectPtrVector;			// 사각형 객체 포인터 벡터
	...
	for (std::vector<RectPtr>::iterator it = rectPtrVector.begin();
		...
		cout << *(*it) << endl;			// RectPtr& rp = *it; cout << *rp << endl;			/ *it = 사각형 객체의 포인터, *(*it): 사각형 객체
		cout << "area : " << (*it)->area() << endl;
	}
	/*
	for (auto it = rectPtrVector.begin(); it != rectPtrVector.end(); ++it) {				// auto로 간편하게 표현
		cout << *(*it) << endl;
		cout << "area : " << (*it)->area() << endl;
	}
	*/
